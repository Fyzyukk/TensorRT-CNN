...
//    auto input_dims  = context->getBindingDimensions(0);
    auto input_dims = context->getTensorShape(engine->getIOTensorName(0));
//    auto output_dims = context->getBindingDimensions(1);
    auto output_dims = context->getTensorShape(engine->getIOTensorName(1));
    ...
    */
    ...
    for (int i = 0; i < input_height; ++i) {
        for (int j = 0; j < input_width; ++j) {
            ...
            input_h[offset_ch1++] = (input_img.data[index + 1] / 255.0f - mean[1]) / std[1];
            input_h[offset_ch0++] = (input_img.data[index + 2] / 255.0f - mean[2]) / std[2];
        }
    }
    timer.stop_cpu();
...
//    if (!context->enqueueV2((void**)bingings, stream, nullptr)) {
    if (!context->executeV2((void**)bingings)){
        LOG("Error happens during DNN inference part, program terminated");
        return false;
    }
    timer.stop_cpu();
    timer.stop_gpu();
    timer.duration_gpu("inference_GPU");
    timer.duration_cpu<Timer::ms>("inference_CPU(enqueuev2)");
